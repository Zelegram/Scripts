...
(async function () {
    const GAME_SETTING = {
        autoPlayNow: true,
        autoPlayAgain: true,
        playAgainDelay: 5000 // milliseconds (5 seconds)
    }
    ...
    function getButton(text, selector = 'button.btn.primary-btn') {
        const targetButton = document.querySelector(selector);
        if (targetButton && targetButton.textContent.trim() === text) {
            return targetButton;
        }
    }
    ...
    function clickPlayNowButton() {
        const interval = setInterval(() => {
            const playNowButton = getButton("Play Now");
            if (playNowButton) {
                playNowButton.click();
                console.log("Play Now button is pressed");
    ...
    function clickPlayAgainButton() {
        setInterval(() => {
            const playAgainButton = getButton("Play Again");
            if (playAgainButton) {
                // Wait 5 seconds before clicking "Play Now"
                setTimeout(() => {
                    playAgainButton.click();
                    console.log(`Play Again button is pressed after ${GAME_SETTING.playAgainDelay} milliseconds`);
                }, GAME_SETTING.playAgainDelay); // 5000 ms = 5 seconds
            }
    ...
    if (GAME_SETTING.autoPlayNow) clickPlayNowButton();

    if (GAME_SETTING.autoPlayAgain) clickPlayAgainButton();
